# VORTEXLOCK Optical Surgery Corpus v1.0.1

## Intended use

VORTEXLOCK is an original synthetic dataset for training and evaluating models that perform decision-focused counterfactual optical control. Given noise-corrupted intensity measurements, a desired topological crossing signature, and eight possible phase-plate controls, a model must choose two distinct controls in order and predict the charged singularity trajectories and detector response produced by that sequence. It supports research in inverse problems, action-conditioned operator learning, structured trajectory prediction, and constrained control. It is not a laboratory calibration standard or a safety-certified laser-control dataset.
...
`train_targets.csv` has 1,024 rows and these ordered columns:

- `scene_id`: training join key.
- `action_json`: JSON object containing distinct, order-sensitive categorical candidate IDs `first` and `second`.
- `braid_json`: JSON object containing six tracks. Each track has integer `charge` (`-1` or `+1`) and `xy`, nine dimensionless `[x,y]` sensor-plane coordinates. Track list order is not semantic.
- `response_json`: JSON array of 32 normalized detector intensities in `[0,1]`. Values are neither probabilities nor optical power in watts.

`sample_submission.csv` has 320 test rows and exactly the same four ordered columns as `train_targets.csv`. Values are valid, nonconstant, label-free examples derived only from public candidate parameters.
...
Every training scene has all 56 ordered pairs. No test landscape is public.
