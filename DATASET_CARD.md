...
- `train_targets.csv` contains the oriented three-control cycle with maximum directed circulation and its closed-loop braid/response holonomies under the exact submission schema.
...
- Absolute landscape responses are normalized detector intensities in `[0,1]`; prepared response targets are signed sums of three order-reversal differences around an oriented cycle. Neither is optical power in watts or a probability. There are 32 angular detectors.
